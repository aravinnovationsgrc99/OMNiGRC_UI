...
const pillars = [
  ...
  },
  {
    ...
    description:
      "Replace annual fire drills with 24/7 automated monitoring. OMNiGRC connects natively to your tech stack, collects audit-ready evidence continuously, and alerts you before exceptions become findings.",
  ...
  },
  {
    ...
    description:
      "Vendor security reviews shouldn't slow down procurement. OMNiGRC autonomously extracts SOC reports, analyzes vendor security posture, flags risks, and keeps your vendor register continuously current.",
...
export const UnifiedPlatformSection: React.FC = () => {
  ...
  return (
    <section className="relative bg-[#0F172A] py-16 sm:py-24 border-t border-slate-800 overflow-hidden">
      ...
      <div className="pointer-events-none absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-[500px] sm:h-[700px] w-[90vw] max-w-[900px] rounded-full bg-brand-orange/10 blur-[100px] sm:blur-[140px]" />
      ...
      <div className="relative z-10 w-full max-w-7xl 2xl:max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-4 gap-6 sm:gap-8 mb-8 sm:mb-12">
          {pillars.map((pillar) => (
            <TiltCard
              key={pillar.id}
              className="p-6 sm:p-8 border-brand-orange/30 bg-slate-900/80 hover:border-brand-orange/60 flex flex-col justify-between micro-card"
            >
              <div>
                <div className="flex items-center justify-between mb-4 gap-2">
                  <span className="text-[10px] font-mono font-bold tracking-wider text-brand-yellow uppercase bg-brand-yellow/10 px-2.5 py-1 rounded-full truncate">
                    {pillar.category}
                  </span>
                  {/* Pulsing Play Button */}
                  <button
                    onClick={() => setSelectedVideo({ title: pillar.videoTitle, open: true })}
                    className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-full bg-brand-orange/20 text-brand-orange border border-brand-orange/40 hover:bg-brand-orange/40 hover:scale-105 transition-all text-xs font-semibold group shrink-0 micro-btn"
                  >
                    <span className="relative flex h-2 w-2">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-brand-orange opacity-75"></span>
                      <span className="relative inline-flex rounded-full h-2 w-2 bg-brand-orange"></span>
                    </span>
                    <Play className="h-3 w-3 sm:h-3.5 sm:w-3.5 fill-brand-orange group-hover:scale-110 transition-transform" /> Watch Demo
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 p-3.5 sm:p-4 rounded-xl bg-slate-950/70 border border-slate-800 mb-6">
                  {pillar.metrics.map((m, mIdx) => (
                    <div key={mIdx}>
                      ...
                      <p className="text-sm sm:text-base font-extrabold text-brand-green">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
          <TiltCard className="p-6 sm:p-8 border-slate-800 bg-slate-900/60 micro-card">
            <div className="flex items-center gap-3 mb-4">
              <div className="p-2.5 rounded-xl bg-brand-orange/20 text-brand-orange shrink-0">
            ...
            <Link href="/products/risk-management" className="text-xs font-semibold text-brand-orange hover:underline flex items-center gap-1">
          ...
          <TiltCard className="p-6 sm:p-8 border-slate-800 bg-slate-900/60 micro-card">
            <div className="flex items-center gap-3 mb-4">
              <div className="p-2.5 rounded-xl bg-brand-green/20 text-brand-green shrink-0">
            ...
            <Link href="/products/unified-commitments" className="text-xs font-semibold text-brand-orange hover:underline flex items-center gap-1">
