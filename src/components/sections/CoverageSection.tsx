...
export const CoverageSection: React.FC = () => {
  ...
  return (
    <section className="relative bg-[#0F172A] py-24 border-t border-slate-800 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-24">
        {/* PART A: FRAMEWORKS */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-5 space-y-6">
            <span className="text-xs font-mono uppercase tracking-widest text-brand-cyan font-semibold">
            ...
            <p className="text-slate-300 text-base sm:text-lg leading-relaxed">
              SOC 2, ISO 27001, HIPAA, GDPR, PCI DSS, DPDP, and 200+ global standards. Upload any custom contract, SLA, or custom security framework — Arav&apos;sGRC translates it into mapped controls automatically.
            </p>
            <div>
              <Link
          ...
          <div className="lg:col-span-7">
            <TiltCard className="p-8 border-brand-teal/30 bg-slate-900/80">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {[
                  ...
                  <div
                    key={idx}
                    className="p-4 rounded-xl border border-slate-800 bg-slate-950/80 hover:border-brand-cyan/50 hover:bg-slate-900 transition-all cursor-pointer group"
                  >
                    <div className="flex items-center justify-between mb-2">
                      <Shield className="h-4 w-4 text-brand-cyan group-hover:scale-110 transition-transform" />
                ...
                {integrationsList.map((app, idx) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="p-3.5 rounded-xl border border-slate-800 bg-slate-950/70 hover:border-brand-cyan/60 hover:bg-slate-900 transition-all flex flex-col items-center text-center cursor-pointer"
                    >
                      <div className="p-2 rounded-lg bg-slate-900 text-brand-cyan mb-2">
                        <Icon className="h-5 w-5" />
                      </div>
                      <p className="font-bold text-xs text-white">{app.name}</p>
                      <p className="text-[9px] text-slate-400 mt-0.5">{app.category}</p>
                    </motion.div>
                  );
                })}
              </div>
