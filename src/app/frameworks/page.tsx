  ...
  return (
    <div className="min-h-screen bg-[#faf8ff] text-[#0d1b36] selection:bg-[#F15E1C]/20 selection:text-[#0d1b36] antialiased flex flex-col justify-between">
      ...
      <main className="flex-1 w-full pt-24 sm:pt-28">
        ...
        <section className="w-full bg-[#f1f3ff] px-4 sm:px-6 lg:px-8 py-12 border-b border-slate-200">
          <div className="max-w-7xl mx-auto">
            <div className="text-center max-w-2xl mx-auto mb-6">
              ...
              <h2 className="text-2xl font-bold text-[#0d1b36] mt-1">Explore Framework Correlations</h2>
            </div>
        ...
        <section className="w-full bg-white px-4 sm:px-6 lg:px-8 py-16 sm:py-20 border-b border-slate-200">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {FRAMEWORKS.map((fw) => (
                <div key={fw.code} className="bg-[#f1f3ff] rounded-2xl p-6 border border-slate-200 shadow-sm hover:shadow-md transition-all flex flex-col justify-between group">
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      <span className="px-2.5 py-1 rounded-full bg-white text-[#0d1b36] font-mono text-[10px] font-bold uppercase border border-slate-200">
                        {fw.region}
                      </span>
                      <span className="font-mono text-xs text-[#006c4d] font-semibold flex items-center gap-1">
                        <CheckCircle2 className="h-3.5 w-3.5" /> Documented
                      </span>
                    </div>

                    <h3 className="text-xl font-bold text-[#0d1b36] mb-1">{fw.name}</h3>
                    <p className="font-mono text-xs text-[#F15E1C] font-semibold mb-3">{fw.headerDesc}</p>
                    <p className="text-xs text-[#5a4138] leading-relaxed mb-4">{fw.desc}</p>

                    {/* Citations Preview */}
                    <div className="p-3 rounded-xl bg-white border border-slate-200 mb-4">
                      <span className="font-mono text-[10px] text-[#5a4138] uppercase font-semibold block mb-1">
                        Sample Framework Citations
                      </span>
                      <div className="flex flex-wrap gap-1.5">
                        {fw.citations.map((c, cIdx) => (
                          <span key={cIdx} className="px-2 py-0.5 rounded bg-[#ffdbcf] text-[#380d00] font-mono text-[10px] font-semibold">
                            {c}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="pt-3 border-t border-slate-200">
                    <Link
                      ...
                      className="text-xs font-bold text-[#F15E1C] hover:text-[#ce4700] inline-flex items-center gap-1 group-hover:translate-x-0.5 transition-transform"
                    >
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 overflow-x-auto">
              <table className="w-full text-left font-body-sm text-xs">
                <thead className="bg-[#f1f3ff] font-mono text-xs text-[#0d1b36] border-b border-slate-200">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-200 text-[#0d1b36]">
                  {FRAMEWORKS.map((fw) => (
                    <tr key={fw.code} className="hover:bg-slate-50 transition-colors">
                      <td className="p-3 font-bold font-mono text-[#F15E1C]">
                        <Link href={`/frameworks/${fw.slug}`} className="hover:underline">
                          {fw.name}
                        </Link>
                      </td>
                      <td className="p-3 text-[#5a4138]">{fw.region}</td>
                      <td className="p-3 text-[#0d1b36]">{fw.headerDesc}</td>
                      <td className="p-3 text-[#006c4d] font-semibold">{fw.oneLiner}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
