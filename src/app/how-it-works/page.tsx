    ...
    <div className="min-h-screen bg-[#faf8ff] text-[#0d1b36] selection:bg-[#F15E1C]/20 selection:text-[#0d1b36] antialiased">
      ...
      <main className="w-full pt-24 sm:pt-28">
        ...
        <section className="relative w-full pt-6 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto text-center">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 mb-10">
            ...
            <a
              ...
              className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white border border-slate-200 text-[#0d1b36] font-semibold hover:bg-slate-50 transition-colors"
            >
          ...
          <div className="w-full bg-white border border-slate-200 rounded-2xl p-5 shadow-sm grid grid-cols-2 md:grid-cols-4 gap-4 text-left">
            <div className="flex flex-col border-r border-slate-200 pr-2">
              <span className="font-mono text-[11px] text-[#5a4138] uppercase tracking-wider">Operational Cadence</span>
              <span className="text-base font-bold text-[#006c4d] flex items-center gap-1 mt-0.5">
                <Clock className="h-4 w-4" /> Scheduled
              </span>
              <span className="text-xs text-[#5a4138]">Audited 30/60/90-day task runs</span>
            </div>
            <div className="flex flex-col border-r border-slate-200 pr-2">
              <span className="font-mono text-[11px] text-[#5a4138] uppercase tracking-wider">Cross-Framework Reuse</span>
              <span className="text-base font-bold text-[#F15E1C] mt-0.5">Map Once</span>
              <span className="text-xs text-[#5a4138]">Across 6 global standards</span>
            </div>
            <div className="flex flex-col border-r border-slate-200 pr-2">
              <span className="font-mono text-[11px] text-[#5a4138] uppercase tracking-wider">Advisory AI</span>
              <span className="text-base font-bold text-[#795600] flex items-center gap-1 mt-0.5">
                <CheckCircle2 className="h-4 w-4" /> Human Review
              </span>
              <span className="text-xs text-[#5a4138]">Mandatory approval on suggestions</span>
            </div>
            <div className="flex flex-col">
              <span className="font-mono text-[11px] text-[#5a4138] uppercase tracking-wider">Audit Records</span>
              <span className="text-base font-bold text-[#0d1b36] mt-0.5 flex items-center gap-1">
                <FileCheck2 className="h-4 w-4 text-[#006c4d]" /> Event Logged
              </span>
              <span className="text-xs text-[#5a4138]">Documented governance trail</span>
            </div>
          </div>
        </section>
        ...
        <section className="w-full py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto" id="lifecycle-stages">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-6 pb-3 border-b border-slate-200">
            <div>
              <span className="px-3 py-1 rounded-full bg-[#ffdbcf] font-mono text-xs text-[#F15E1C] font-semibold uppercase">
                Operating Architecture
              </span>
              <h2 className="text-2xl sm:text-4xl text-[#0d1b36] font-bold tracking-tight mt-2">
                The 9-Stage Operating Lifecycle
              </h2>
              <p className="text-xs sm:text-sm text-[#5a4138] mt-1">
                Risk → Assets → Controls → Policies → Compliance → Audits → Evidence → Remediation → Governance Intelligence
              </p>
            </div>
            <div className="mt-3 md:mt-0 flex items-center gap-2 font-mono text-xs text-[#006c4d] font-semibold">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-9 gap-2 mb-6 bg-[#f1f3ff] p-2.5 rounded-2xl border border-slate-200">
            {stages.map((s) => (
              <button
                key={s.id}
                ...
                className={`py-2.5 px-2 rounded-xl text-center transition-all font-mono flex flex-col items-center gap-0.5 ${
                  activeStage === s.id
                    ...
                    : "bg-white text-[#0d1b36] hover:bg-slate-50 border border-slate-200"
                }`}
              >
            ...
            <div className="lg:col-span-7 bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between mb-4">
                  <span className="font-mono text-xs font-bold text-[#F15E1C] uppercase tracking-wider bg-[#ffdbcf] px-3 py-1 rounded-full">
                    STAGE {activeData.num} • {activeData.badge}
                  </span>
                  <span className="text-xs font-semibold text-[#006c4d] flex items-center gap-1">
                    <CheckCircle2 className="h-4 w-4" /> Operational Stage
                  </span>
                </div>

                <h3 className="text-xl sm:text-2xl font-bold text-[#0d1b36] mb-3">
                  {activeData.title}
                </h3>

                <p className="text-sm text-[#5a4138] leading-relaxed mb-6">
                  {activeData.desc}
                </p>

                <div className="space-y-3 bg-[#f1f3ff] p-5 rounded-xl border border-slate-200">
                  <span className="font-mono text-xs text-[#0d1b36] font-bold block uppercase tracking-wider mb-2">
                    Key Deliverables &amp; Operational Guardrails
                  </span>
                  {activeData.points.map((pt, pIdx) => (
                    <div key={pIdx} className="flex items-start gap-2.5 text-xs text-[#0d1b36]">
                      <CheckCircle2 className="h-4 w-4 text-[#006c4d] shrink-0 mt-0.5" />
                      <span>{pt}</span>
                    </div>
                  ))}
                </div>
              ...
              <div className="mt-8 pt-4 border-t border-slate-200 flex items-center justify-between">
                <span className="text-xs font-mono text-[#5a4138]">
                  Tenant Isolated • Human Governed Workflow
                </span>
                <Link href="/demo" className="text-xs font-bold text-[#F15E1C] hover:underline inline-flex items-center gap-1">
        ...
        <section className="w-full py-12 px-4 sm:px-6 lg:px-8 bg-[#f1f3ff] border-y border-slate-200">
          <div className="max-w-7xl mx-auto">
            <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-8">
                <div>
                  <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-[#95f6cb] text-[#00513a] font-mono text-xs font-semibold mb-2">
                    <CheckCircle2 className="h-4 w-4" /> OPERATING PRINCIPLE
                  </span>
                  <h2 className="text-2xl sm:text-3xl font-bold text-[#0d1b36]">AI Assists. Humans Decide.</h2>
                  <p className="text-sm text-[#5a4138] max-w-2xl mt-1 leading-relaxed">
                ...
                <div className="shrink-0 p-4 rounded-xl bg-[#f1f3ff] border border-slate-200 flex items-center gap-3">
                  <Shield className="h-8 w-8 text-[#F15E1C]" />
                  <div>
                    <span className="block text-xs font-bold text-[#0d1b36]">Zero Autonomous Writes</span>
                    <span className="block font-mono text-[11px] text-[#5a4138]">Mandatory Human Gatekeeper</span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div className="bg-[#faf8ff] p-5 rounded-xl border border-slate-200">
                  <div className="flex items-center gap-2 mb-3 text-[#006c4d]">
                    <CheckCircle2 className="h-5 w-5" />
                    <h3 className="text-base font-bold text-[#0d1b36]">Sanitized Advisory Payloads</h3>
                  </div>
                  <ul className="space-y-3 text-xs text-[#0d1b36]">
                ...
                <div className="bg-[#faf8ff] p-5 rounded-xl border border-slate-200">
                  <div className="flex items-center gap-2 mb-3 text-[#F15E1C]">
                    <Lock className="h-5 w-5" />
                    <h3 className="text-base font-bold text-[#0d1b36]">Mandatory Human Gatekeeper</h3>
                  </div>
                  <ul className="space-y-3 text-xs text-[#0d1b36]">
                    <li className="flex items-start gap-2">
                      <Lock className="h-4 w-4 text-[#F15E1C] shrink-0 mt-0.5" />
        ...
        {/* CTA Section */}
        <section className="w-full bg-[#faf8ff] px-4 sm:px-6 lg:px-8 py-16 text-center border-t border-slate-200">
          <div className="max-w-4xl mx-auto">
            ...
            <p className="text-sm sm:text-base text-[#5a4138] mb-8">
